...
# --- VALIDATION ---
if (-not $CollectLocal -and -not $LogPath) {
    Write-Error "You must either provide a -LogPath OR use the -CollectLocal switch."
    Exit
}
...
function Get-ErrorInsight {
    ...
    $kb = @{
        "-2016281112" = @{ 
            Hint = "Generic Win32 MSI Failure."; 
            DocUrl = "https://learn.microsoft.com/en-us/mem/intune/apps/troubleshoot-app-install";
            MvpUrl = "https://patchmypc.com/blog/powershell-script-installer-support-for-win32-apps-in-intune/"; MvpName = "Patch My PC";
            PortalUrl = "https://intune.microsoft.com/#view/Microsoft_Intune_DeviceSettings/AppsWindowsMenu/~/windowsApps"; PortalName = "Intune Apps Blade"
        }
        "0x800705b4"  = @{ 
            Hint = "TPM/ESP Timeout. Hardware issue."; 
            DocUrl = "https://learn.microsoft.com/en-us/windows/deployment/windows-autopilot/troubleshoot-autopilot-errors#0x800705b4";
            MvpUrl = "https://andrewstaylor.com/2022/08/16/autopilot-troubleshooting-tools-during-esp/"; MvpName = "Andrew Taylor";
            PortalUrl = "https://intune.microsoft.com/#view/Microsoft_Intune_DeviceSettings/DevicesWindowsEnrollmentMenu/~/windowsAutopilot"; PortalName = "Autopilot Devices"
        }
        "AAD token"   = @{ 
            Hint = "Azure AD comms failed. Check Network/TPM."; 
            DocUrl = "https://learn.microsoft.com/en-us/autopilot/troubleshoot-device-enrollment";
            MvpUrl = "https://patchmypc.com/blog/entra-join-no-password-prompt-user-realm-issue/"; MvpName = "Patch My PC";
            PortalUrl = "https://entra.microsoft.com/#view/Microsoft_AAD_Devices/DevicesMenuBlade/~/DevicesAll"; PortalName = "Entra ID Devices"
        }
        "channel URI" = @{ 
            Hint = "Push Notification failed."; 
            DocUrl = "https://learn.microsoft.com/en-us/mem/intune/enrollment/troubleshoot-windows-enrollment-errors";
            MvpUrl = "https://andrewstaylor.com/2024/09/02/enrolling-windows-devices-into-intune-a-definitive-guide/"; MvpName = "Andrew Taylor";
            PortalUrl = "https://intune.microsoft.com/#view/Microsoft_Intune_DeviceSettings/DevicesWindowsEnrollmentMenu/~/windowsAutopilot"; PortalName = "Autopilot Devices"
        }
        "Timeout"     = @{ 
            Hint = "ESP exceeded 60m limit."; 
            DocUrl = "https://learn.microsoft.com/en-us/mem/intune/enrollment/troubleshoot-esp-timeout";
            MvpUrl = "https://patchmypc.com/blog/why-do-required-apps-wait-60-minutes-after-autopilot-enrollment/"; MvpName = "Patch My PC";
            PortalUrl = "https://intune.microsoft.com/#view/Microsoft_Intune_Enrollment/DeviceEnrollmentMenuBlade/~/enrollmentStatusPage"; PortalName = "Intune ESP Profiles"
        }
        "Defender"    = @{
            Hint = "Security Baseline or Defender Onboarding failed.";
            DocUrl = "https://learn.microsoft.com/en-us/microsoft-365/security/defender-endpoint/troubleshoot-onboarding";
            MvpUrl = "https://call4cloud.nl/category/microsoft-defender/"; MvpName = "Call4Cloud (Defender)";
            PortalUrl = "https://security.microsoft.com/"; PortalName = "Defender Security Center"
        }
    }
    ...
    foreach ($line in [System.IO.File]::ReadLines($ime.FullName)) {
        if ($line -match $rex) {
            ...
            if ($m.Component -match "(?i)AppWorkload|AgentExecutor" -and $m.Message -match "(?i)Downloading|Installing|Enforcing|ExitCode") {
                ...
                $apps += [PSCustomObject]@{ Time=$m.Time; Component=$m.Component; Status=$st; Message=$m.Message }
            }
        }
    }
}
